...
{
    return I2C_HAL_MasterSendDataPolling(mpu_dev.instance,
        mpu_dev.addr, &addr, 1, &val, 1);
}
...
static uint8_t read_reg(uint8_t addr)
{
    ...
    I2C_HAL_MasterReceiveDataPolling(mpu_dev.instance,
        mpu_dev.addr >> 1, &addr, 1, &val, 1);
    return val;
}

static uint8_t i2c_read_reg(I2C_Type *instance, uint8_t addr, uint8_t reg, uint8_t *val)
{
    return I2C_HAL_MasterReceiveDataPolling(instance, addr,
        &reg, 1, val, 1);
}

// 在I2C总线上产生9个SCL时钟脉冲, 解锁总线
static void i2c_unlock(void)
{
    ...
    GPIO_HAL_SetPinDir(PTC, 10u, kGpioDigitalOutput);
    GPIO_HAL_SetPinDir(PTC, 11u, kGpioDigitalOutput);
    GPIO_HAL_WritePinOutput(PTC, 10, 0);
    GPIO_HAL_WritePinOutput(PTC, 11, 0);
    
    // 9个SCL时钟
    for (i = 0; i < 9; ++i) {
        GPIO_HAL_WritePinOutput(PTC, 10, 0);
        delay_ms(1);
        GPIO_HAL_WritePinOutput(PTC, 10, 1);
        delay_ms(1);
    }
    GPIO_HAL_WritePinOutput(PTC, 10, 0);
}
...
static void i2c_init(void)
{
    ...
    i2c_unlock(); // 先解锁总线, 否则在单片机复位(不是重新上电)之后MPU6050很容易卡死
    PORT_HAL_SetMuxMode(PORTC, 10u, kPortMuxAlt2);
    PORT_HAL_SetOpenDrainCmd(PORTC, 10u, true); // 设置成开漏模式
    PORT_HAL_SetPullMode(PORTC, 10u, kPortPullUp); // 引脚上拉
    PORT_HAL_SetMuxMode(PORTC, 11u, kPortMuxAlt2);
    PORT_HAL_SetOpenDrainCmd(PORTC, 11u, true); // 设置成开漏模式
    PORT_HAL_SetPullMode(PORTC, 11u, kPortPullUp); // 引脚上拉
    
    SIM_HAL_EnableClock(SIM, kSimClockGateI2c1);
    I2C_HAL_Init(I2C1);
    I2C_HAL_Enable(I2C1);
    I2C_HAL_SetBaudRate(I2C1, BSP_GetBusClockFreq(), 200, NULL);
    I2C_HAL_ClearInt(I2C1);
    
    /* Set direction to send. */
    I2C_HAL_SetDirMode(I2C1, kI2CSend);
}
...
int mpu6050_init(void)
{
    ...
    i2c_init();
    mpu_dev.instance = I2C1;
    
    MPU6050_TRACE("MPU6050 initialization...\n");
    for(i = 0; i < ARRAY_SIZE(mpu_addr); i++)
    {
        if(!i2c_read_reg(I2C1, mpu_addr[i], WHO_AM_I, &id))
        {
            if(id == 0x68)
            {
                ...
                /* init sequence */
                write_reg(PWR_MGMT_1, 0x00); // 复位MPU6050
                write_reg(SMPLRT_DIV, 0x02); // 采样分频 div2(500Hz)
                write_reg(CONFIG, 0x00);
                write_reg(AUX_VDDIO, 0x80);
                write_reg(GYRO_CONFIG, 3 << 3); // 陀螺仪量程 ±2000dps
                write_reg(ACCEL_CONFIG, 0x00); //加速度传感器 ±2g
    ...
    MPU6050_TRACE("MPU6050 not found.\n");
    return 1;
}
...
{
    uint8_t err, cmd = ACCEL_XOUT_H;
    uint8_t buf[6];
    
    err = I2C_ReceiveBuffer(mpu_dev.instance, mpu_dev.addr, cmd, buf, 6);
